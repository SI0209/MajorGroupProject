...
namespace DrivingSchoolWebsite
{
    ...
    public class ChatbotController : WebService
    {
        ...
        // MAIN ENDPOINT — returns full FAQ HTML
        [WebMethod]
        ...
        // Build structured FAQ layout
        private string BuildFaqHtml()
        {
            ...
            foreach (var category in FaqData)
            {
                ...
                html.Append($"<div class='faq-title'>{category.Key}</div>");

                foreach (var item in category.Value)
                {
                    html.Append("<div class='faq-item'>");
                    html.Append($"<div class='faq-question'>{item.Key}</div>");
                    html.Append($"<div class='faq-answer'>{item.Value}</div>");
                    html.Append("</div>");
                }
